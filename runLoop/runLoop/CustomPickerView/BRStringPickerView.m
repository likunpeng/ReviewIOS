...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    UIView *itemView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 1.0f, 200.0f, 38.0f)];
    itemView.backgroundColor = UIColor.whiteColor;
    UIImageView *countryImgView = [[UIImageView alloc] init];
    countryImgView.image = [UIImage imageNamed:@"img02"];
    [itemView addSubview:countryImgView];
    [countryImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(itemView).offset(10.0f);
        make.centerY.equalTo(itemView);
        make.width.height.mas_equalTo(30.0f);
    }];

    UILabel *countryNameLabel = [[UILabel alloc] init];
    countryNameLabel.text = _dataSource[row];
    NSLog(@"country name = %@", _dataSource[row]);
    countryNameLabel.textColor = UIColor.blackColor;
    [itemView addSubview:countryNameLabel];
    [countryNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(countryImgView.mas_right).offset(20.0f);
        make.centerY.equalTo(itemView);
